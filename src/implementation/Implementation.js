  ...
  render() {
    return (
      <>
        ...
        <p>
          We just generate a random number and return it. Now, let's compare v2 and v3. In v2 we call the <Code inline>slow_rand</Code> function. This takes 30 seconds to complete, which is blocking the response thread.
        </p>
        <div className="EmbeddedCode" ref={ this.version2Ref } />
        <p>
          Now let's take a look at v3, and see what the differences are.
        </p>
        ...
        <p>
          With full socket hijacking we are responsible for making sure the response to the client is well-formed. In our solution you can see on lines 21-35 we need to write the code that handles the HTTP response status line and headers.
        </p>
        ...
        <p>
          In our controller, at line 23, we use <Code inline>ensure</Code> to make sure the socket is always closed. However, if our application was doing more complicated things, like storing sockets while awaiting a subscription event, it would be a great idea to make sure to limit the number of sockets we hijack and keep open at any one time.
        </p>
